...
// parse band info
exports.parseBandInfo = function (html, bandUrl) {
  var $ = cheerio.load(html);
  var data = scrapeIt.scrapeHTML($, {
    head: {
      selector: 'head',
      data: {
        ...
        bio: { selector: "meta[property='og:description']", attr: 'content' },
        ...
        imageUrl: { 
          ...
          convert: function (src) {
            if (src) {
              return src.replace(/_\d{1,3}\./, "_20."); // use small version
            }
          } 
        },
      }
  ...
  var object = assignProps(data.head, {}, ['name', 'bio', 'url', 'imageUrl']);
